...
def p_namespace(p):
    ...
    # namespace is useless in _shaded_thriftpy
    # if p[2] == 'py' or p[2] == '*':
...
def parse(path, module_name=None, include_dirs=None, include_dir=None,
    ...
    if url_scheme == 'file':
        ...
        raise ThriftParserError('_shaded_thriftpy does not support generating module '
    ...
    if module_name is not None and not module_name.endswith('_thrift'):
        raise ThriftParserError('_shaded_thriftpy can only generate module with '
                                '\'_thrift\' suffix')
...
def parse_fp(source, module_name, lexer=None, parser=None, enable_cache=True):
    """Parse a file-like object to thrift module object, e.g.::
        ...
        >>> from _shaded_thriftpy.parser.parser import parse_fp
