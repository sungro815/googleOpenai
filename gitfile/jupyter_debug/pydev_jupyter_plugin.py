...
def can_not_skip(plugin, pydb, pydb_frame, frame, info):
    ...
    if pydb.jupyter_breakpoints:
        ...
        if filename in cell_info.jupyter_cell_name_to_id:
            cell_id = cell_info.jupyter_cell_name_to_id[filename]
...
def _is_inside_jupyter_cell(frame, pydb):
    while frame is not None:
        ...
        if is_cell_filename(filename):
            if filename not in pydb.cell_info.jupyter_cell_name_to_id and filename not in pydb.cell_info.util_cell_names:
...
def _convert_filename(frame, pydb):
    filename = frame.f_code.co_filename
    if is_cell_filename(filename):
        return pydb.cell_info.jupyter_cell_name_to_id[filename]
    else:
        return filename
